//! Barter [`AlgoStrategy`] implementation that bridges WASM strategies with the barter engine.
//!
//! Receives 1-minute candle data from [`CandleInstrumentData`], aggregates to the target
//! [`Timeframe`], and delegates signal generation to a [`StrategyHandle`] running in WASM.
...
pub type WasmEngineState = EngineState<DefaultGlobalData, CandleInstrumentData>;

/// Barter strategy that delegates signal generation to a WASM-compiled strategy.
///
/// Aggregates 1-minute candles from [`CandleInstrumentData`] into the target [`Timeframe`],
/// then calls the WASM strategy's `on_candles()` when a new aggregated bar completes.
...
impl WasmAlgoStrategy {
    /// Create a new WASM-backed strategy.
    ///
    /// # Arguments
    /// * `handle` - Executable WASM strategy handle
    ...
    pub fn new(handle: Box<dyn StrategyHandle>, timeframe: Timeframe) -> Self {
        Self {
            id: StrategyId::new("wasm"),
            state: RefCell::new(WasmStrategyInner {
                handle,
                timeframe,
                buffer: Vec::new(),
                history: Vec::new(),
...
impl AlgoStrategy for WasmAlgoStrategy {
    type State = WasmEngineState;
            ...
            for candle in new_candles {
                ...
                if should_aggregate(&inner.buffer, inner.timeframe) {
                    ...
                    match inner.handle.on_candles(&history_snapshot) {
                        ...
                        Err(err) => {
                            warn!(%err, "WASM strategy on_candles failed");
                        }
                    }
                }
            }
...
impl ClosePositionsStrategy for WasmAlgoStrategy {
    type State = WasmEngineState;
...
impl<Clock, ExecutionTxs, Risk> OnDisconnectStrategy<Clock, WasmEngineState, ExecutionTxs, Risk>
    for WasmAlgoStrategy
{
    type OnDisconnect = ();

    fn on_disconnect(
        _: &mut Engine<Clock, WasmEngineState, ExecutionTxs, Self, Risk>,
        _: ExchangeId,
    ) -> Self::OnDisconnect {
    }
}

impl<Clock, ExecutionTxs, Risk> OnTradingDisabled<Clock, WasmEngineState, ExecutionTxs, Risk>
    for WasmAlgoStrategy
{
