...
/// Factory function that creates a [`BrokerConfig`] from a raw config map.
pub type CreateConfigFn =
    fn(&HashMap<String, String>) -> std::result::Result<BrokerConfig, BrokerRegistryError>;
...
pub struct BrokerRegistryEntry {
    ...
    pub create_broker: CreateBrokerFn,
    /// Create a [`BrokerConfig`] from a raw config map.
    pub create_config: CreateConfigFn,
}

/// Global broker registry, populated by [`crate::brokers::register_all`].
pub static BROKER_REGISTRY: std::sync::LazyLock<Vec<BrokerRegistryEntry>> =
    std::sync::LazyLock::new(crate::brokers::register_all);

/// Look up a broker registry entry by its type key.
pub fn find_broker(type_key: &str) -> Option<&'static BrokerRegistryEntry> {
    BROKER_REGISTRY.iter().find(|e| e.type_key == type_key)
}
